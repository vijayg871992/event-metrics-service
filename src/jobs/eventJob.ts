...
export function startEventJob(appLog?: { info: Function; error: Function }) {
  const worker = new Worker(
    ...
    async (job: Job) => {
      ...
      await BatchModel.updateOne({ batch_id: batchId }, { status: 'processing' });

      const events = await EventModel.find({ batch_id: batchId, processed: false });
      if (!events.length) {
  ...
  worker.on('failed', async (job, err) => {
    ...
    if (job?.data?.batchId) {
      await BatchModel.updateOne({ batch_id: job.data.batchId }, { status: 'failed' });
    }
  });
