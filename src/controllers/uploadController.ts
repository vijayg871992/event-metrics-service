...
import { checkUploadRate } from "../utils/rateLimiter";
...
export async function handleUpload(
  ...
  try {
    ...
    const ip =
      ...
      request.ip;
    const allowed = await checkUploadRate(ip);

    if (!allowed) {
      request.log.error(`Rate limit exceeded for IP: ${ip}`);
      return reply
        .status(429)
        .send({ error: "Only 1 upload allowed per minute. Please wait." });
    }
    ...
    request.log.error(`Upload failed: ${err.message}`);
